...
public interface UserMangMappingRepository extends AbstractBaseRepository<UserMangMappingEntity, Long> {
	...
	@Query(value = "select count(userroleid) cnt from c_user_role_page_nav_map_mgmt where userroleid =?1 and page_id =?2 and parent_page_id =?3", nativeQuery = true)
	int getCountOfMappedRecord(int roleid, int pageid, int parentpageid);
